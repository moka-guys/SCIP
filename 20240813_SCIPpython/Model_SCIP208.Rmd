...
# Read the HBB mpileup output into a vector where each element is a line
lines <- readLines("perloutputHBB.txt")
# Remove the last 5 lines
lines <- head(lines, -5)
# Write the modified content back to new file
writeLines(lines, "perloutputHBB_short.txt")


FL_SNPs <- read.table("/cloud/project/perloutputHBB_short.txt", sep="\t", header = T)
# rename columns
...
```
```{r, include=FALSE}
#Add the Total count and the Alternative count for the disease allele in question
# patient SCIP259's maternal genotype is SC - need to look at both alleles independently, remember they're at different loci
